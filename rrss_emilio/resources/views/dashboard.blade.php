                        ...
                        <div>
                            ...
                            ('image_id', $image->id)->isEmpty())
                                <svg data-id="{{$image->id}}"
                                     ...
                                     width="24"
                                     height="24"
                                     fill="currentColor" class="bi bi-balloon-heart like"
                                     viewBox="0 0 16 16">
                            ...
                            @endif
                            <p><span class="count"></span> likes</p>
...
<script>
    ...
    window.addEventListener('load', () => {
        heart.forEach((h, index) => {
            const token = h.dataset.token
            const user = h.dataset.user
            const data = {
                image_id: h.dataset.id,
                user_id: user
            }
            fetch(`http://rrss.emilio.com/dashboard/countLikes`, {
                method: 'POST',
                ...
                .then((res) => res.json())
                .then((data) => {
                    count[index].textContent = data.count
                })
        })
    })
    ...
    heart.forEach((h, index) => {
        ...
        h.addEventListener('click', () => {
            h.classList.toggle('stroke-red-700')
            const data = {
                image_id: h.dataset.id,
                user_id: user
            }
            if (!h.classList.contains('stroke-red-700')) {
                fetch(`http://rrss.emilio.com/dashboard/dislike`, {
                    method: 'POST',
                    body: JSON.stringify(data),
                    mode: 'cors',
                    headers: {
                    ...
                    .then((res) => res.json())
                    .then((data) => count[index].textContent = data.count)
            } else {
                fetch(`http://rrss.emilio.com/dashboard/like`, {
                    method: 'POST',
                    body: JSON.stringify(data),
                    headers: {
                        "X-CSRF-TOKEN": token,
